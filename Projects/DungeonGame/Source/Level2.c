...
void Level2Init(void)
{
	...
	GameObject* enemy2 = GameObjectCreate(levelCenter.x - 5.0f, levelCenter.y - 7.0f, 'X', COLOR_RED, EnemyUpdate, NULL);
	...
	GameObjectAddCollisionPair(player, enemy2, PlayerCollisionEnemy);
...
void Level2Update(float dt)
{
	...
	EngineDrawText("LEVEL 2", COLOR_GREEN, levelCenter.x - 3.0f, levelCenter.y - 12.0f);
}
