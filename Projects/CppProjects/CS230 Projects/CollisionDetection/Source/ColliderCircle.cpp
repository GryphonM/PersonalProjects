...
bool ColliderCircle::IsCollidingWith(const Collider& other) const
{
	...
	{
		CircleCircleIntersection(Beta::Circle(transform->GetTranslation(), radius),
	...
	{
		PointCircleIntersection(other.transform->GetTranslation(),
			Beta::Circle(transform->GetTranslation(), radius));
	}
	else if (other.GetType() == ColliderType::Rectangle)
	{
		RectangleCircleIntersection(Beta::BoundingRectangle(other.transform->GetTranslation(), 
